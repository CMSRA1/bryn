...
for dir in range(0,len(DirKeys)):
  for hist in HistNames[dir]:
    ...
    Zinv = GetHist("../results/AK5Calo_Zinvisible_jets.root",91,1,"Z->Invisiable")
    ...
    herr.GetYaxis().SetRangeUser(0.00001,MaxY*1.2)
    ...
    lumi = Root.TLatex(0.4,.85,"#int L dt = " + str(intlumi) + "pb^{-1}, #sqrt{s} = 7 TeV");
    prelim.SetNDC()
    lumi.SetNDC()
    # prelim.Draw()
    lumi.Draw()
    ...
    if "AlphaT_0" in hist :
      ...
      if (DirKeys[dir].GetTitle())[0] != "n":
        ...
        Images ='''<tr>
          <td><a href="'''+(DirKeys[dir].GetTitle())+"_Log_"+hist+'.png\"><img src=\"'+(DirKeys[dir].GetTitle())+"_Log_"+hist+'.png\" width =\"598\" height=\"286\" /></a></td>'"<td><a href=\""+"n"+(DirKeys[dir].GetTitle())+"_Log_"+hist+'.png\"><img src=\"'+"n"+(DirKeys[dir].GetTitle())+"_Log_"+hist+'.png\" width =\"598\" height=\"286\" /></a></td>'
        Webpage.write(Images)
      # mergedPlots.writeTObject(c1)
      c1.cd(1).SetLogy()
      # c1.cd(2)
