...
// #include "QuantileError.h"
  ...
  gROOT->ForceStyle();
  TFile *A = new TFile("AK5JPT_ICSkimTest.root");
  // TFile *A = new TFile("AK5Calo_SD_JMT.root");

  TFile *B = new TFile("AK5JPT_MatchingMC.root");
  //
  // TFile *A = new TFile("AK5Calo_Skims_clean.root");
  // TFile *B = new TFile("AK5Calo_MatchingMC.root");
    ...
    // TH1D *oneside = new TH1D("oneisde","side1",80,0.,400);
      ...
      for(Int_t i=1; i<=a; i++) {
        ...
        if(i < px1->FindBin(0) ) {
          ...
          // abs8->SetBinContent(i,temp16+temp16);
          ...
          // absm8->SetBinContent(i,tempm16+tempm16);
     ...
     cout << errorM4[o] << "error on M4" << endl;
        ...
        TGraphErrors *graphM3 = new TGraphErrors(16, range1,sigQuantM3,0 ,errorM3);
        TGraphErrors *graphM4 = new TGraphErrors(16, range3,sigQuantM4,0 ,errorM4);
      ...
      // L1->AddEntry( graph1, "Uncorrected Calo  #slashed{E}_{T}y" ,"P");
      // L1->AddEntry( graphM1, "Uncorrected Calo #slashed{E}_{T}y (MC)" ,"P");
      // L1->AddEntry( graph2, "TCMETy" ,"P");
      L1->AddEntry( graph3, "Calo  #slashed{E}_{T} Type-1 corrected y (Data)" ,"P");
      L1->AddEntry( graphM3, "Calo  #slashed{E}_{T} Type-1 corrected y (MC)" ,"P");
      // L1->AddEntry( graph4, "PFMET y" ,"P");
      L1->AddEntry( graph5, "#slash{H}_{T} from Type-1 Corrected Jets x (Data)" ,"P");
      L1->AddEntry( graphM5, "#slash{H}_{T} from Type-1 Corrected Jets x (MC)" ,"P");
      ...
      mg->Add(graphM5,"P");
      mg->Draw("a");
      mg->GetXaxis()->SetTitle("#sigma expected");
      mg->GetYaxis()->SetTitle("#sigma observed");
      ...
      // TPad *aPad = new TPad();//"padname","somestring",0,0,1024,640);
      // aPad->SetTransparent(30);
      // aPad->cd();
      graph5->SetMarkerStyle(16);
      graphM5->SetMarkerStyle(16);
      graphM5->SetMarkerColor(kAzure);
      graphM5->SetLineColor(kAzure);
      graph3->SetMarkerStyle(22);
      graph3->SetFillColor(1);
      graphM3->SetMarkerStyle(22);
      graphM3->SetLineColor(kAzure);
      graphM3->SetMarkerColor(kAzure);
      pfy->Add(graph5,"P");
      pfy->Add(graphM5,"P");
      pfy->Add(graph3,"P");
      pfy->Add(graphM3,"P");
      ...
      pfy->Draw("a");
      pfy->GetXaxis()->SetTitle("#sigma_{gaus}");
      pfy->GetYaxis()->SetTitle("#frac{quantile(#sigma_{gaus})}{#sigma_{core}}");
      pfy->GetYaxis()->SetRangeUser(0.,7.);
         L1->Clear();
          L1->AddEntry(graph5, "#slash{H}_{T} (Data)","P");
          L1->AddEntry(graphM5, "#slash{H}_{T} (MC)","P");
          L1->AddEntry(graph3, "#slash{E}_{T} (Data)","P");
          L1->AddEntry(graphM3, "#slash{E}_{T} (MC)","P");
          L1->AddEntry(xy, "Gaussian", "L");
          L1->Draw();
          c1->Modified();
      c1->SaveAs("JPTQuantitiesYaxis.pdf");
      c1->Clear();
      gROOT->ForceStyle();
      ...
      px5->SetMarkerStyle(20);
      px5->GetXaxis()->SetTitle("#slash{H}_{T}x,y");
      px5->GetXaxis()->SetRangeUser(-180.,180.);
      // pm5->SetMarkerColor(4);
      pm5->SetFillColor(kAzure-2);
      pm5->SetLineColor(kBlue+2);
      pm5->GetFunction("gam5")->SetLineColor(kRed);
      pm5->GetFunction("gam5")->SetLineStyle(1);
            pm5->GetFunction("gam5")->SetLineWidth(2);
            px5->GetXaxis()->SetTitleSize(0.048);
            pm5->GetXaxis()->SetRangeUser(-160.,160.);
      // px5->Draw("Psame");
