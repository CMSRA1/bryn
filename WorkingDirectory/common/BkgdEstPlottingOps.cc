...
//----------------------------------------------------------------------
mSuGraPlottingOps::mSuGraPlottingOps(  const Utils::ParameterSet& ps ) :  dirName_( ps.Get<std::string>("DirectoryName") )
{}
...
void mSuGraPlottingOps::BookHistos() {
  ...
  BookHistArray( H_M0_M12_MHTovHT, 
		 "m0_m12_MHTovHT", 
		 ";m0;m12",
		 80,0,4000,//m0
		 25,100,600,//m12
  ...
  BookHistArray( H_M0_M12_ThrustDeltaPhi, 
		 "m0_m12_ThrustDeltaPhi", 
		 ";m0;m12",
		 80,0,4000,//m0
		 25,100,600,//m12
		  3, 0, 1,false );

  BookHistArray( H_M0_M12_MissingTransverse, 
		 "m0_m12_MissingTransverse", 
		 ";m0;m12",
		 80,0,4000,//m0
		 25,100,600,//m12
		  3, 0, 1,false );
...
bool mSuGraPlottingOps::Process( Event::Data& ev ) {
  ...
  if(ev.JD_CommonJets().accepted.size() > 0) DPhiOne = ROOT::Math::VectorUtil::DeltaPhi(ev.CommonRecoilMET(),*(*ijet));
  ...
  // cout << " thrust delta phi" << var2_ThrustDeltaPhi << endl;
  ...
  try{//works only for mSuGra scan files
    ...
    //all jet mult
    H_M0_M12[0]->Fill(ev.M0(),ev.M12(),weight);
    H_M0_M12_noweight[0]->Fill(ev.M0(),ev.M12(),1);

    if(var2_alphaT > 0.55){
      H_M0_M12_alphaT[0]->Fill(ev.M0(),ev.M12(),weight);
    }
    if(var2_MHTovHT > 0.5){
      H_M0_M12_MHTovHT[0]->Fill(ev.M0(),ev.M12(),weight);
    }
    if(var2_ThrustDeltaPhi > 1){
      H_M0_M12_ThrustDeltaPhi[0]->Fill(ev.M0(),ev.M12(),weight);
    }
    if(var2_MissingTransverse > 250){
      H_M0_M12_MissingTransverse[0]->Fill(ev.M0(),ev.M12(),weight);
    }  
    
    //Fill MHT eta HT plots (RA2 cuts)
    if(fabs(DPhiOne) >= 0.3 && fabs(DPhiTwo) >= 0.5 && fabs(DPhiThree) >= 0.3 && var2_MHT > 200  ){
      H_M0_M12_MHT[0]->Fill(ev.M0(),ev.M12(),weight);
    }

    //dijet events
    if(numJets == 2){
      H_M0_M12[1]->Fill(ev.M0(),ev.M12(),weight);
      H_M0_M12_noweight[1]->Fill(ev.M0(),ev.M12(),1);
      
      if(var2_alphaT > 0.55){
	H_M0_M12_alphaT[1]->Fill(ev.M0(),ev.M12(),weight);
      }
      if(var2_ThrustDeltaPhi > 1){
	H_M0_M12_ThrustDeltaPhi[1]->Fill(ev.M0(),ev.M12(),weight);
      }
      if(var2_MHTovHT > 0.5){
	H_M0_M12_MHTovHT[1]->Fill(ev.M0(),ev.M12(),weight);
      }
      if(var2_MissingTransverse > 250){
	H_M0_M12_MissingTransverse[1]->Fill(ev.M0(),ev.M12(),weight);
      }  
      
      //Fill MHT eta HT plots (RA2 cuts)
      if(fabs(DPhiOne) >= 0.3 && fabs(DPhiTwo) >= 0.5 && fabs(DPhiThree) >= 0.3 && var2_MHT > 200  ){
	H_M0_M12_MHT[1]->Fill(ev.M0(),ev.M12(),weight);
      } 
    }

    //>2 jet events
    if(numJets > 2){
      H_M0_M12[2]->Fill(ev.M0(),ev.M12(),weight);
      H_M0_M12_noweight[2]->Fill(ev.M0(),ev.M12(),1);
      
      if(var2_alphaT > 0.55){
	H_M0_M12_alphaT[2]->Fill(ev.M0(),ev.M12(),weight);
      }
      if(var2_ThrustDeltaPhi > 1){
	H_M0_M12_ThrustDeltaPhi[2]->Fill(ev.M0(),ev.M12(),weight);
      }
      if(var2_MHTovHT > 0.5){
	H_M0_M12_MHTovHT[2]->Fill(ev.M0(),ev.M12(),weight);
      }
      if(var2_MissingTransverse > 250){
	H_M0_M12_MissingTransverse[2]->Fill(ev.M0(),ev.M12(),weight);
      }  
      
      //Fill MHT eta HT plots (RA2 cuts)
      if(fabs(DPhiOne) >= 0.3 && fabs(DPhiTwo) >= 0.5 && fabs(DPhiThree) >= 0.3 && var2_MHT > 200  ){
	H_M0_M12_MHT[2]->Fill(ev.M0(),ev.M12(),weight);
      } 
    }
...
bool BkgdEstPlottingOps::Process( Event::Data& ev ) {
  ...
  Double_t DPhiThree=99; 
    ...
    try{//works only for mSuGra scan files
      int m0_feld = (int)(ev.M0()/50+0.5);
      int m12_feld = (int)((ev.M12()-100)/20+0.5);

      // if(m0_feld !=  m0_feld_save)  cout << " m0 " << ev.M0() << " feld " << m0_feld << " m12 " << ev.M12() << " feld " << m12_feld << endl;  
      m0_feld_save = m0_feld;

      if(m0_feld < m0_felder_ && m12_feld < m12_felder_){
	  
	//Fill alphaT eta HT plots
	if(var2_alphaT != -1)  alphaT_vs_etaLeading_HT[m0_feld][m12_feld]->Process(weight,var1,var2_alphaT,var3,numJets); 	 
	...
	if(fabs(DPhiOne) >= 0.3 && fabs(DPhiTwo) >= 0.5 && fabs(DPhiThree) >= 0.3){
	  MHT_vs_etaLeading_HT[m0_feld][m12_feld]->Process(weight,var1,var2_MHT,var3,numJets);
      	...
      	//Fill alphaT eta HT plots
	if(var2_alphaT != -1)  alphaT_vs_etaLeading_HT[0][0]->Process(weight,var1,var2_alphaT,var3,numJets); 	 
...
bool ABCDMethod::Process(Double_t weight, Double_t var1, Double_t var2, Double_t var3,Int_t numJets){
  ...
  for(int i = 0; i < ABCD_TH2D.size(); i++){
