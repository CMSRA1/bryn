  ...
  if(PhotJetindex != -1){
	...
	//	hCCMuJet_DeltaPtovPt_[1]->Fill(((ev.genJetP4()->at(genIndex)).Pt() -  ((ev.JD_CommonJets().accepted).at((*ijet).GetIndex()))->Pt())/((ev.genJetP4()->at(genIndex)).Pt()));
     ...
     if(PhotJetDeltaR < 0.5){//if ele jet in 0.5 cone
      ...
      hCCPhotJet_CombRelIso_[0]->Fill( ((ev.PD_Photons())[PhotJetindex]).GetAllIsolation() /  ((ev.PD_Photons())[PhotJetindex]).Pt(),weight);
     

     }else if(PhotJetindex != -1)hCCPhotJet_CombRelIso_[1]->Fill( ((ev.PD_Photons())[PhotJetindex]).GetAllIsolation() /  ((ev.PD_Photons())[PhotJetindex]).Pt(),weight);
