  ...
  if ( StandardPlots_ ){
    ...
    if(!ev.pthat.enabled()){
      // if(ev.CommonAlphaT() > 0.55 || HadronicCommonPlots::MT2_Leading(ev) > 200. || HadronicCommonPlots::MT2(ev) > 200. || ev.CommonMHT().Pt() > 250.){
        ...
        cout << "run : " <<  ev.RunNumber() << "lumi Section "<< ev.LumiSection() <<  " event : "<<  ev.EventNumber() << " Alpha T is" << ev.CommonAlphaT() << " Effective mass is : " << ev.CommonMHT().Pt()+ev.CommonHT() << " Dijet Mass is " << (lv2).M() << endl;
        cout << "x-cleaned"<<endl;
